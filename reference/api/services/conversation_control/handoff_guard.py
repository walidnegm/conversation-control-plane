...
def would_ping_pong(
    ...
    *,
    ...
    if not trace:
        return False
    last = trace[-1]
    last_fr = last.get("from") or ""
    last_to = last.get("to") or ""
    # Last transition was B→A; proposing A→B again.
    if last_fr == to and last_to == fr:
        return True
    # Same directed pair twice inside the recent window.
    window = trace[-4:]
    pair_hits = sum(
        1 for t in window
