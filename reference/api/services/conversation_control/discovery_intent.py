...
_DISCOVERY_CLASSIFIER_INLINE = (
    ...
    "NEVER platform_catalog for marketplace asks.\n"
    "- marketplace_query: ONLY when kind=agent_marketplace AND the user named a "
    "searchable domain, capability, vendor, or product. Concise search terms "
    "only — not the full utterance, not list/browse filler words. Leave \"\" when "
    "the ask is inventory/browse/overview of the marketplace with no search "
    "target (code serves an overview). Never invent search terms from the whole "
    "utterance. Empty when kind≠agent_marketplace.\n"
